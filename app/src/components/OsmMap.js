...
const HTML = (lat, lng, safeRadius, accent) => `
<!DOCTYPE html>
<html><head>
...
<script>
  ...
  L.circle([${lat}, ${lng}], { radius: ${safeRadius}, color: '${accent}', weight: 1.5, dashArray: '4 4', fillOpacity: 0.06 }).addTo(map);
...
export default function OsmMap({ lat, lng, safeRadius = 150, height = 220 }) {
  if (lat == null || lng == null) {
    return <View style={{ height, backgroundColor: "#1b2334" }} />;
  }
  const html = HTML(lat, lng, safeRadius, C.accent);
